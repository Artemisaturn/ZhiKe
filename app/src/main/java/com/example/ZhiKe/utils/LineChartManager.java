...
public class LineChartManager {
    ...
    private void initLineChart(boolean legendShow) {
        ...
        lineChart.setDragEnabled(true);//false    // 可拖动,默认true
        lineChart.setScaleEnabled(true); //false  // 两个轴上的缩放,X,Y分别默认为true
        lineChart.setScaleXEnabled(true); //false // X轴上的缩放,默认true
        lineChart.setScaleYEnabled(true);//false  // Y轴上的缩放,默认true
        lineChart.setPinchZoom(true);//false  // X,Y轴同时缩放，false则X,Y轴单独缩放,默认false
        lineChart.setDoubleTapToZoomEnabled(true);//false // 双击缩放,默认true
...
//        设置线的宽度
        ...
        leftAxis.setTextColor(Color.DKGRAY);//parseColor("#d5d5d5"));
...
//        设置网格为虚线
    ...
    private void initLineDataSet(LineDataSet lineDataSet, int color, boolean mode) {
        ...
        // 不显示具体值
        lineDataSet.setDrawValues(true);//false
...
//        xAxis.setLabelCount(xAxisValues.size(), false);
        xAxis.setTextColor(Color.BLACK);//parseColor("#a1a1a1"));
